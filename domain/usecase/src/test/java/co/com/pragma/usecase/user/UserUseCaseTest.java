...
//        when(userGateway.existUserByEmailAndDocument(user.getEmail())).thenReturn(Mono.just(false));
...
//        when(userGateway.existUserByEmailAndDocument(user.getEmail())).thenReturn(Mono.just(true));
//
//        Mono<User> result = userUseCase.createUser(user);
//
//        StepVerifier.create(result)
